...
function aggWH(data) {
  const m = {};
  data.forEach(r => {
    r.warehouse.split(',').map(w => w.trim()).forEach(wh => {
  ...
  arr.sort((a, b) => {
    const na = parseInt(a.warehouse.match(/\d+/)?.[0] || '0');
    const nb = parseInt(b.warehouse.match(/\d+/)?.[0] || '0');
    return na - nb;
  });
...
function aggBR(data) {
  ...
  data.forEach(r => {
    ...
    r.warehouse.split(',').map(w => w.trim()).forEach(wh => b.whMap[wh] = (b.whMap[wh] || 0) + 1);
...
function aggJT(data) {
  ...
  data.forEach(r => {
    ...
    r.warehouse.split(',').map(w => w.trim()).forEach(wh => v.whMap[wh] = (v.whMap[wh] || 0) + 1);
