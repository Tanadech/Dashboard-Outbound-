...
function normalizeData(json) {
  ...
  const cWH   = fcol(H, ['คลังสินค้า', '^คลัง$']);
  ...
  const cRec  = fcol(H, ['ผู้บันทึก.*T3', 'T3.*ผู้บันทึก']);
  const cRecT4 = fcol(H, ['ผู้บันทึก.*T4', 'T4.*ผู้บันทึก', 'ผู้บันทึก']);
  ...
  return json.map(r => {
    ...
    return {
      ...
      warehouse: cWH
        ? (String(r[cWH] || '').trim() || 'ไม่ระบุคลัง')
        : normalizeWarehouse(cT2 ? r[cT2] : '', cT3 ? r[cT3] : ''),
      ...
      recorderT4: cRecT4 ? (String(r[cRecT4] || '').trim() || '')               : ''
    };
  }).filter(r => r.docNo !== '' && /^WH[1-9]\d*$/i.test(r.warehouse));
}
